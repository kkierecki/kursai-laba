  ...
  try {
    ...
    const result = streamText({
      ...
      onFinish: ({ finishReason, usage }) => {
        void recordApiUsage(usageContext.database, usageContext.user.id, usage, "gemini-3.1-flash-lite", "/api/format").catch((error) =>
          logTechnical("ERROR", "api-usage.write.failed", { route: "/api/format", requestId: requestLog.requestId, error }),
        );
