...
export default function DashboardPage() {
  ...
  return <main className="dashboard-shell runner-dashboard">
    ...
    <section className="dashboard-grid" aria-label="Podsumowanie biegacza">
      <article className="dashboard-card goal-card"><DashboardHeading icon="goal" label="Najbliższy cel" />{goal ? <><strong className="goal-title">{goal.title}</strong><div className="goal-details">{goal.target_value !== null && <p><DashboardIcon name="pace" />{goal.target_metric}: {goal.target_value} {goal.target_unit}</p>}{goal.target_date && <p><DashboardIcon name="calendar" />Termin: {new Date(`${goal.target_date}T00:00:00`).toLocaleDateString("pl-PL")}</p>}{goalDetail && <p className="goal-description">{goalDetail}</p>}</div></> : <p>Nie masz aktywnego celu. Opisz go trenerowi w czacie.</p>}<section className="latest-workout-summary" aria-label="Ostatni trening"><div className="latest-workout-heading"><DashboardIcon name="workout" /><span>Ostatni trening</span></div>{workout ? <><strong>{workout.summary}</strong><p className="workout-date"><DashboardIcon name="calendar" />{new Date(`${workout.performed_on}T00:00:00`).toLocaleDateString("pl-PL")}</p><dl className="latest-workout-metrics"><DashboardMetric icon="distance" label="Dystans" value={workout.distance_m ? `${(workout.distance_m / 1000).toFixed(1)} km` : "—"} /><DashboardMetric icon="pace" label="Tempo" value={pace(workout.average_pace_seconds)} /><DashboardMetric icon="heart" label="Śr. HR" value={workout.average_hr ?? "—"} /></dl></> : <p>Brak zapisanego treningu.</p>}</section></article>
