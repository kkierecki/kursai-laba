...
export async function searchKnowledge(query: string, userId?: string, database = supabase) {
  const normalized = query.trim();
  ...
  try {
    ...
    const { data, error } = await database.rpc("match_documents", { query_embedding: embedding, match_threshold: 0.5, match_count: 5, match_user_id: userId });
